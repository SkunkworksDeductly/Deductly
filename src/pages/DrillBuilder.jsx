...
const SingleSelectDropdown = ({
  ...
  return (
    <div className="relative space-y-2" ref={containerRef}>
      ...
      <button
        type="button"
        className="w-full rounded-lg px-3 py-2 text-left text-white flex items-center justify-between bg-transparent hover:bg-white/10 transition"
        onClick={() => setIsOpen((prev) => !prev)}
      >
        <span className={`truncate text-sm ${selectedOption ? 'text-white' : 'text-white/60'}`}>
          {summary}
        </span>
        <span className="ml-3 text-white/60 text-xs">v</span>
      </button>

      {isOpen && (
        <div className="absolute left-0 z-20 mt-2 w-full rounded-xl bg-slate-900/95 backdrop-blur shadow-lg">
          <div className="py-2">
            {options.map((option) => {
            ...
            {options.length === 0 && (
              <div className="px-3 py-2 text-sm text-white/50">No options</div>
            )}
          </div>
        </div>
      )}
    ...
    <div className="py-10">
      <div className="max-w-6xl mx-auto px-4 space-y-8">
        <div className="rounded-2xl border-0 p-8">
          <div className="mb-6">
            <h1 className="text-3xl md:text-4xl font-bold text-white mb-2">Build Your LSAT Drill</h1>
          ...
          <div className="grid gap-6 lg:grid-cols-2">
            ...
            <div className="space-y-6">
              <div className="rounded-xl border-0 bg-transparent p-5 space-y-4">
                <h3 className="text-lg font-semibold text-white">Drill Summary</h3>
                <div className="space-y-2 text-sm text-white/70">
                  <p>
                    <span className="font-semibold text-white/80">Questions:</span> {drillConfig.questionCount}
                  </p>
                  <p>
                    <span className="font-semibold text-white/80">Difficulty mix:</span> {drillConfig.difficulties.join(', ')}
                  </p>
                  <p>
                    <span className="font-semibold text-white/80">Timing:</span>{' '}
                    {
                  ...
                  <p>
                    <span className="font-semibold text-white/80">Skills:</span>{' '}
                    {drillConfig.skills.length > 0 ? drillConfig.skills.join(', ') : 'Any available'}
                  </p>
                </div>
                {errorMessage && (
                  <div className="text-sm text-red-400 bg-red-500/15 rounded-lg px-3 py-2">
                    {errorMessage}
                  </div>
                )}
                <button
                  type="button"
                  className="w-full py-2.5 bg-matte-red hover:bg-matte-red/90 rounded-lg text-white font-semibold transition disabled:opacity-60 disabled:cursor-not-allowed"
                ...
                <button
                  type="button"
                  className="w-full py-2 bg-white/10 hover:bg-white/20 rounded-lg text-white/80 text-sm transition"
        ...
        <div className="bg-transparent rounded-2xl border-0 p-8 text-white">
          <h2 className="text-2xl font-semibold mb-3">Ready when you are</h2>
          <p className="text-white/70 max-w-2xl">
            Choose your drill configuration above and tap <span className="text-white font-semibold">Generate Drill</span> to pull a fresh set of questions from the Deductly skill library.
          </p>
          <div className="mt-6 grid gap-4 md:grid-cols-3 text-sm text-white/70">
            <div className="rounded-xl border-0 bg-transparent p-4">
              <p className="text-white font-semibold">Adaptive mix</p>
              <p className="mt-2 text-white/60">We'll fill any gaps in your request with similar questions so every drill reaches your requested length.</p>
            </div>
            <div className="rounded-xl border-0 bg-transparent p-4">
              <p className="text-white font-semibold">Real LSAT content</p>
              <p className="mt-2 text-white/60">Questions map directly to LSAT reasoning skills so you always know what you're sharpening.</p>
            </div>
            <div className="rounded-xl border-0 bg-transparent p-4">
              <p className="text-white font-semibold">Flexible pacing</p>
              <p className="mt-2 text-white/60">Dial the timer from tight to untimed to mirror your practice strategy.</p>
            </div>
          </div>
        </div>
      </div>
